...
class BoardState:
    ...
    def copyLoggingState(self):
        ...
        for player in self.players:
            copyPlayers.append(player.copyPlayer())
        ret = LoggingBoardState(copyPlayers, False, self.module, False)
        ret.temple = copy.deepcopy(self.temple)
        ret.shields = copy.deepcopy(self.shields)
        ret.chaos = copy.deepcopy(self.chaos)
        ret.dogged = copy.deepcopy(self.dogged)
        ...
        match self.phase:
            ...
            case Phase.MINOR_RESOLVE_EFFECTS:
                if move[0] == Move.CHOOSE_ADD_HAMMER_SCEPTER:
                    ...
                    if self.players[self.blessingPlayer].gainMinorBlessingEffect(self.cyclops,
                                                                                 self.blessing) and self.oracle:
                        for player in self.players:
    ...
    def getOptions(self):
        ...
        match self.phase:
            ...
            case Phase.FORGE_BOAR_MISFORTUNE_1 | Phase.FORGE_BOAR_MISFORTUNE_2:
                ret = self.generateForgeBoarMisfortuneFace()
            case Phase.SATYRS_CHOOSE_DIE_1:
    ...
    def generateForgeBoarMisfortuneFace(self):
        ret = []
        for existingFace in self.players[self.blessingPlayer].die1.faces:
            if not Data.isBoarFace(existingFace):
                ret.append((Move.FORGE_FACE, self.blessingPlayer, (True, self.faceToForge, existingFace)))
        for existingFace in self.players[self.blessingPlayer].die2.faces:
            if not Data.isBoarFace(existingFace):
                ret.append((Move.FORGE_FACE, self.blessingPlayer, (False, self.faceToForge, existingFace)))
        ret = set(ret)  # remove duplicates
        return tuple(ret)
    ...
    def celestialDieRandomRoll(self):
        return random.choice((Data.CelestialDieFace.CELESTIAL12GOLD, Data.CelestialDieFace.CELESTIAL5VP,
                              Data.CelestialDieFace.CELESTIAL3VPAND3GOLD1SUN1MOONOR,
                              Data.CelestialDieFace.CELESTIALGODDESS,
    ...
    def observation(self):  # encode board state
        ...
        while True:
            ...
            obs.extend([p.maxGold / (12.0 + 4 * len(self.players)), p.maxSun / (6.0 + 3 * len(self.players)),
                        p.maxMoon / (6.0 + 3 * len(self.players)), ])
            ...
            obs.extend([p.orChoice1 / 5.0, p.orChoice2 / 5.0, p.orMazeChoice1 / 5.0, p.orMazeChoice2 / 5.0,
                        p.celestialOrChoice / 5.0, ])
            ...
            obs.extend(
                [p.location / 7.0, p.numForged / 20.0, p.allegiance / 15.0, p.mazePosition / 35.0, p.mazeMoves / 2.0,
                 p.shipsToResolve / 2.0, p.times3ShipsToResolve / 2.0, p.celestialRolls / 2.0,
                 p.hammerTrack / (30.0 * len(self.players)), p.goldToGain / 12.0, p.goldToSpend / 12.0,
                 p.sunToSpend / 6.0, p.moonToSpend / 6.0, ])
...
class LoggingBoardState:
    def __init__(self, players, initialState, module, printingEnabled):
        self.players = players
        self.module = module  # 0 is no module, 1 is goddess, 2 is titans
        self.temple = ([Data.DieFace.GOLD3, Data.DieFace.GOLD3, Data.DieFace.GOLD3, Data.DieFace.GOLD3],
                       [Data.DieFace.MOON1, Data.DieFace.MOON1, Data.DieFace.MOON1, Data.DieFace.MOON1],
    ...
    def copyLoggingState(self):
        ...
        for player in self.players:
            copyPlayers.append(player.copyPlayer())
        ret = LoggingBoardState(copyPlayers, False, self.module, self.printingEnabled)
        ret.temple = copy.deepcopy(self.temple)
        ret.shields = copy.deepcopy(self.shields)
        ret.chaos = copy.deepcopy(self.chaos)
        ret.dogged = copy.deepcopy(self.dogged)
    ...
    def togglePrinting(self):
        self.printingEnabled = not self.printingEnabled
    ...
    def makeMove(self, move):
        ...
        if move[0] == Move.USE_TRITON_TOKEN:
            if self.printingEnabled:
                print(f"Player {move[1]} uses a Triton token to gain {move[2][0]}")
            if self.loggingEnabled:
                self.log.write(f"Player {move[1]} uses a Triton token to gain {move[2][0]}\n")
            match move[2][0]:
                case "sun":
                    self.players[move[1]].gainSun(2, False)
                case "moon":
        ...
        if move[0] == Move.USE_COMPANION:
            if self.printingEnabled:
                print(f"Player {move[1]} uses a Companion to gain {move[2][0]} sun and vp")
            if self.loggingEnabled:
                self.log.write(f"Player {move[1]} uses a Companion to gain {move[2][0]} sun and vp\n")
            self.players[move[1]].gainSun(move[2][0], False)
            self.players[move[1]].gainVP(move[2][0])
            self.players[move[1]].companions.remove(move[2][0])
            return
        match self.phase:
            ...
            case Phase.TWINS_REROLL_CHOICE:
                if move[0] == Move.TWINS_CHOOSE_DIE:
                    if move[2][0]:
                        if self.printingEnabled:
                            print(f"Player {move[1]} chooses to use the effect of The Twins to reroll die 1")
                        if self.loggingEnabled:
                            self.log.write(f"Player {move[1]} chooses to use the effect of The Twins to reroll die 1\n")
                        self.phase = Phase.TWINS_REROLL_1
                    else:
                        if self.printingEnabled:
                            print(f"Player {move[1]} chooses to use the effect of The Twins to reroll die 2")
                        if self.loggingEnabled:
                            self.log.write(f"Player {move[1]} chooses to use the effect of The Twins to reroll die 2\n")
                        self.phase = Phase.TWINS_REROLL_2
            case Phase.TWINS_REROLL_1:
                if move[0] == Move.ROLL:
                    if self.printingEnabled:
            ...
            case Phase.USE_CERBERUS_CHOICE:
                if move[0] == Move.USE_CERBERUS:
                    if move[2][0]:
                        if self.printingEnabled:
                            print(f"Player {move[1]} uses a Cerberus token")
                        if self.loggingEnabled:
                            self.log.write(f"Player {move[1]} uses a Cerberus token\n")
                        self.players[self.blessingPlayer].cerberusTokens -= 1
                        self.cerberus = True
                    self.players[self.blessingPlayer].setBuffers()
                    self.phase = Phase.MIRROR_1_CHOICE
                    ...
                    if self.blessingPlayer == self.activePlayer:  # everyone ousted
                        ...
                        for feat in self.players[self.activePlayer].feats:
                            if feat == Data.HeroicFeat.GREAT_BEAR:
                                if self.printingEnabled:
                                    print(
                                        f"Player {self.blessingPlayer} gains 3 vp due to the automatic effect of The Great Bear")
                                if self.loggingEnabled:
                                    self.log.write(
                                        f"Player {self.blessingPlayer} gains 3 vp due to the automatic effect of The Great Bear\n")
                        self.players[self.blessingPlayer].checkBears()
            ...
            case Phase.MINOR_CHOOSE_DIE:
                if move[0] == Move.CHOOSE_DIE:
                    if move[2][0]:
                        ...
                        die = 2
                    if self.printingEnabled:
                        print(f"Player {move[1]} chooses to gain a minor blessing using their die {die}")
                    if self.loggingEnabled:
                        self.log.write(f"Player {move[1]} chooses to gain a minor blessing using their die {die}\n")
                    self.players[self.blessingPlayer].dieChoice = move[2][0]
                    self.phase = Phase.MINOR_ROLL_DIE
            case Phase.MINOR_ROLL_DIE:
                if move[0] == Move.ROLL:
                    ...
                    if self.players[self.blessingPlayer].gainMinorBlessingEffect(self.cyclops,
                        ...
                        for player in self.players:
                            if player.playerID != self.blessingPlayer:
                                if self.printingEnabled:
                                    print(
                                        f"Player {player.playerID} loses 1 loyalty due to the reinforcement effect of The Oracle")
                                if self.loggingEnabled:
                                    self.log.write(
                                        f"Player {player.playerID} loses 1 loyalty due to the reinforcement effect of The Oracle\n")
                                player.allegiance = Data.getPrev(player.allegiance)
            ...
            case Phase.RESOLVE_MERCHANT_REINF:
                if move[0] == Move.MERCHANT_UPGRADE:
                    ...
                    if len(move[2]) > 1:
                        ...
                        if self.printingEnabled:
                            print(
                                f"Player {move[1]} upgrades the die face {move[2][2]} to the die face {move[2][3]} on their die {die} due to the reinforcement effect of The Merchant")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {move[1]} upgrades the die face {move[2][2]} to the die face {move[2][3]} on their die {die} due to the reinforcement effect of The Merchant\n")
                        self.temple[Data.getPool(move[2][3])].remove(move[2][3])
            ...
            case Phase.RESOLVE_LIGHT_REINF:
                if move[0] == Move.USE_LIGHT:
                    if move[2][0]:
                        ...
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {move[1]} spends 3 gold to resolve the effect of the die face {move[2][1]}\n")
                        self.players[self.activePlayer].gainGold(-3, False)
                        self.players[self.activePlayer].die1ResultBuffer = move[2][1]
                        self.players[self.activePlayer].dieChoice = True
                        self.blessingPlayer = self.activePlayer
                ...
                if move[0] == Move.GUARDIAN_CHOICE:
                    match move[2][0]:
                        case "ancientshard":
                            if self.printingEnabled:
                                print(
                                    f"Player {move[1]} gains 1 ancient shard due to the reinforcement effect of The Guardian")
                            if self.loggingEnabled:
                                self.log.write(
                                    f"Player {move[1]} gains 1 ancient shard due to the reinforcement effect of The Guardian\n")
                            self.players[self.activePlayer].gainAncientShards(1)
                        case "loyalty":
                            if self.printingEnabled:
                                print(
                                    f"Player {move[1]} gains 1 loyalty due to the reinforcement effect of The Guardian")
                            if self.loggingEnabled:
                                self.log.write(
                                    f"Player {move[1]} gains 1 loyalty due to the reinforcement effect of The Guardian\n")
                            self.players[self.activePlayer].gainLoyalty(1)
            ...
            case Phase.ACTIVE_PLAYER_CHOICE_1:
                if move[0] == Move.CHOOSE_BUY_FACES:
                    if self.printingEnabled:
                        print(f"Player {move[1]} chooses to buy faces")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} chooses to buy faces\n")
                    self.phase = Phase.ACTIVE_PLAYER_BUY_FACES_1
                elif move[0] == Move.CHOOSE_PERFORM_FEAT:
                    if self.printingEnabled:
                        print(f"Player {move[1]} chooses to perform a feat")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} chooses to perform a feat\n")
                    self.phase = Phase.ACTIVE_PLAYER_PERFORM_FEAT_1
            case Phase.ACTIVE_PLAYER_CHOICE_2:
                if move[0] == Move.CHOOSE_BUY_FACES:
                    if self.printingEnabled:
            ...
            case Phase.EXTRA_TURN_DECISION:
                if move[0] == Move.TAKE_EXTRA_TURN:
                    if move[2][0]:
                        if self.printingEnabled:
                            print(f"Player {move[1]} spends 2 sun to perform an extra action")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {move[1]} spends 2 sun to perform an extra action\n")
                        self.players[self.activePlayer].gainSun(-2, False)
                        self.phase = Phase.ACTIVE_PLAYER_CHOICE_2
                    else:
                        self.phase = Phase.END_TURN
            ...
            case Phase.ACTIVE_PLAYER_BUY_FACES_1:
                if move[0] == Move.BUY_FACES:
                    for face in move[2]:
                        if self.printingEnabled:
                            print(f"Player {move[1]} buys the die face {face} for {Data.faceCosts[face]} gold")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {move[1]} buys the die face {face} for {Data.faceCosts[face]} gold\n")
                        self.temple[Data.getPool(face)].remove(face)
                        self.players[self.activePlayer].buyFace(face)
                elif move[0] == Move.FORGE_FACE:
                    if move[2][0]:
            ...
            case Phase.TRIDENT_1:
                if move[0] == Move.BUY_FACES:
                    ...
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} gains the die face {move[2][0]} due to the instant effect of The Abyssal Trident")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} gains the die face {move[2][0]} due to the instant effect of The Abyssal Trident\n")
                    self.players[self.activePlayer].unforgedFaces.append(move[2][0])
            ...
            case Phase.GODDESS_CHOOSE_FACES:
                if move[0] == Move.CHOOSE_FACES:
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} sets their dice to the faces {move[2][0]} and {move[2][1]} due to the instant effect of The Goddess")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} sets their dice to the faces {move[2][0]} and {move[2][1]} due to the instant effect of The Goddess\n")
                    self.players[self.activePlayer].die1.setToFace(move[2][0])
            ...
            case Phase.WIND_CHOOSE_RESOURCE:
                if move[0] == Move.CHOOSE_RESOURCE:
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} chooses the resource type {Data.getResourceType(move[2][0])} to gain due to the instant effect of The Wind")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} chooses the resource type {Data.getResourceType(move[2][0])} to gain due to the instant effect of The Wind\n")
                    for player in self.players:
            ...
            case Phase.CELESTIAL_CHOOSE_OR:
                if move[0] == Move.CHOOSE_ADD_HAMMER_SCEPTER:
                    ...
                    if move[2][0] == 0:
                        if self.printingEnabled:
                            print(
                                f"Player {self.celestialPlayer} gains 3 gold due to the effect of the CELESTIAL3VPAND3GOLD1SUN1MOONOR die face")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {self.celestialPlayer} gains 3 gold due to the effect of the CELESTIAL3VPAND3GOLD1SUN1MOONOR die face\n")
                        self.players[self.celestialPlayer].gainGold(3, False)
                        ...
                        if self.printingEnabled:
                            print(
                                f"Player {self.celestialPlayer} gains 1 sun due to the effect of the CELESTIAL3VPAND3GOLD1SUN1MOONOR die face")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {self.celestialPlayer} gains 1 sun due to the effect of the CELESTIAL3VPAND3GOLD1SUN1MOONOR die face\n")
                        self.players[self.celestialPlayer].gainSun(1, False)
                        ...
                        if self.printingEnabled:
                            print(
                                f"Player {self.celestialPlayer} gains 1 moon due to the effect of the CELESTIAL3VPAND3GOLD1SUN1MOONOR die face")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {self.celestialPlayer} gains 1 moon due to the effect of the CELESTIAL3VPAND3GOLD1SUN1MOONOR die face\n")
                        self.players[self.celestialPlayer].gainMoon(1, False)
            ...
            case Phase.CELESTIAL_CHOOSE_UPGRADE:
                if move[0] == Move.CELESTIAL_UPGRADE:
                    ...
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} upgrades the die face {move[2][1]} to the die face {move[2][2]} on their die {die} due to the effect of the CELESTIALUPGRADE die face")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} upgrades the die face {move[2][1]} to the die face {move[2][2]} on their die {die} due to the effect of the CELESTIALUPGRADE die face\n")
                    self.temple[Data.getPool(move[2][2])].remove(move[2][2])
            ...
            case Phase.CELESTIAL_CHOOSE_MIRROR:
                if move[0] == Move.CELESTIAL_MIRROR_CHOICE:
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} uses the effect of the die face CELESTIALMIRROR to copy the die face {move[2][0]}")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} uses the effect of the die face CELESTIALMIRROR to copy the die face {move[2][0]}\n")
                    self.players[self.celestialPlayer].celestialResultBuffer = move[2][0]
            ...
            case Phase.CELESTIAL_CHOOSE_GODDESS:
                if move[0] == Move.CELESTIAL_GODDESS:
                    if move[2][0]:
                        if self.printingEnabled:
                            print(
                                f"Player {move[1]} uses the effect of the die face CELESTIALGODDESS to place the face {move[2][1]} on their die 1 face up and resolve its effect")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {move[1]} uses the effect of the die face CELESTIALGODDESS to place the face {move[2][1]} on their die 1 face up and resolve its effect\n")
                        self.players[self.celestialPlayer].die1.setToFace(move[2][1])
                        ...
                        if self.printingEnabled:
                            print(
                                f"Player {move[1]} uses the effect of the die face CELESTIALGODDESS to place the face {move[2][1]} on their die 2 face up and resolve its effect")
                        if self.loggingEnabled:
                            self.log.write(
                                f"Player {move[1]} uses the effect of the die face CELESTIALGODDESS to place the face {move[2][1]} on their die 2 face up and resolve its effect\n")
                        self.players[self.celestialPlayer].die2.setToFace(move[2][1])
            ...
            case Phase.CELESTIAL_RESOLVE_DIE_OR:
                if move[0] == Move.CHOOSE_DIE_OR:
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} chooses to gain {Data.getResourceType(move[2][0])} from their die face {self.players[self.celestialPlayer].celestialResultBuffer}")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} chooses to gain {Data.getResourceType(move[2][0])} from their die face {self.players[self.celestialPlayer].celestialResultBuffer}\n")
                    self.players[self.celestialPlayer].celestialOrChoice = move[2][0]
            ...
            case Phase.CELESTIAL_RESOLVE_DIE_EFFECT:
                if move[0] == Move.CHOOSE_ADD_HAMMER_SCEPTER:
                    ...
                    if self.printingEnabled:
                        print(
                            f"Player {move[1]} applies the effect of the die face {self.players[self.celestialPlayer].celestialResultBuffer}")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {move[1]} applies the effect of the die face {self.players[self.celestialPlayer].celestialResultBuffer}\n")
                    self.players[self.celestialPlayer].gainCelestialDieEffects()
    ...
    def resolveInstEffect(self, effect):
        match effect:
            case "SPIRITS_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains 3 gold and 3 moon due to the instant effect of Wild Spirits")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 3 gold and 3 moon due to the instant effect of Wild Spirits\n")
                if self.players[self.activePlayer].scepters:
            ...
            case "TRITON_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 1 Triton token due to the instant effect of Triton")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 1 Triton token due to the instant effect of Triton\n")
                self.players[self.activePlayer].tritonTokens += 1
                self.makeMove((Move.PASS, self.activePlayer, ()))
            case "MIRROR_INST":
                self.players[self.activePlayer].unforgedFaces.append(Data.DieFace.MIRROR)
            ...
            case "CYCLOPS_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 4 minor blessings due to the instant effect of Cyclops")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 4 minor blessings due to the instant effect of Cyclops\n")
                self.extraRolls = 3
                self.cyclops = True
                if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1:
                    self.returnPhase = Phase.EXTRA_TURN_DECISION
            ...
            case "SPHINX_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 4 minor blessings due to the instant effect of Sphinx")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 4 minor blessings due to the instant effect of Sphinx\n")
                self.extraRolls = 3
                if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1:
                    self.returnPhase = Phase.EXTRA_TURN_DECISION
                else:
            ...
            case "TYPHON_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains {self.players[self.activePlayer].numForged} vp due to the instant effect of Typhon")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains {self.players[self.activePlayer].numForged} vp due to the instant effect of Typhon\n")
                self.players[self.activePlayer].gainVP(self.players[self.activePlayer].numForged)
            ...
            case "SENTINEL_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 2 divine blessings due to the instant effect of Sentinel")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 2 divine blessings due to the instant effect of Sentinel\n")
                self.extraRolls = 1
                self.sentinel = True
                if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1:
                    self.returnPhase = Phase.EXTRA_TURN_DECISION
            ...
            case "CANCER_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 2 divine blessings due to the instant effect of Cancer")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 2 divine blessings due to the instant effect of Cancer\n")
                self.extraRolls = 1
                if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1:
                    self.returnPhase = Phase.EXTRA_TURN_DECISION
                else:
            ...
            case "CERBERUS_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 1 Cerberus token due to the instant effect of Cerberus")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 1 Cerberus token due to the instant effect of Cerberus\n")
                self.players[self.activePlayer].cerberusTokens += 1
                self.makeMove((Move.PASS, self.activePlayer, ()))
            case "BOAR_INST_AUTO_RED":
                self.faceToForge = Data.DieFace.REDBOAR
            ...
            case "CHEST_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} expands their inventory space due to the instant effect of The Blacksmith's Chest")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} expands their inventory space due to the instant effect of The Blacksmith's Chest\n")
                self.players[self.activePlayer].chestEffect()
            ...
            case "HAMMER_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains a hammer track due to the instant effect of The Blacksmith's Hammer")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains a hammer track due to the instant effect of The Blacksmith's Hammer\n")
                self.makeMove((Move.PASS, self.activePlayer, ()))  # handled elsewhere
            case "NYMPH_INST":
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 4 gold due to the instant effect of The Wood Nymph")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 4 gold due to the instant effect of The Wood Nymph\n")
                self.players[self.activePlayer].gainGold(4, False)
                if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1 or self.phase == Phase.TITAN_1:
                    self.phase = Phase.NYMPH_1
                else:
            ...
            case "OMNISCIENT_INST":
                ...
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains {2 * vpFaces} vp due to the instant effect of The Omniscient")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains {2 * vpFaces} vp due to the instant effect of The Omniscient\n")
                self.players[self.activePlayer].gainVP(2 * vpFaces)
            ...
            case "GOLDSMITH_INST":
                ...
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains {2 * len(feats)} vp due to the instant effect of The Goldsmith")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains {2 * len(feats)} vp due to the instant effect of The Goldsmith\n")
                self.players[self.activePlayer].gainVP(2 * len(feats))
            ...
            case "TRIDENT_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} spends all of their gold due to the instant effect of The Abyssal Trident")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} spends all of their gold due to the instant effect of The Abyssal Trident\n")
                self.players[self.activePlayer].gold = 0
            ...
            case "LEFTHAND_INST":
                if self.printingEnabled:
                    print(f"All players are ousted due to the instant effect of The Abyssal Trident")
                if self.loggingEnabled:
                    self.log.write(f"All players are ousted due to the instant effect of The Abyssal Trident\n")
                for player in self.players:
                    if player.location != 0 and player.playerID != self.activePlayer:
                        # another player will be ousted, so activate bears for active player
                        for feat in self.players[self.activePlayer].feats:
                            if feat == Data.HeroicFeat.GREAT_BEAR:
                                if self.printingEnabled:
                                    print(
                                        f"Player {self.activePlayer} gains 3 vp due to the automatic effect of The Great Bear")
                                if self.loggingEnabled:
                                    self.log.write(
                                        f"Player {self.activePlayer} gains 3 vp due to the automatic effect of The Great Bear\n")
                        self.players[self.activePlayer].checkBears()
            ...
            case "TITAN_INST":
                if self.islands[0] or self.islands[1] or self.islands[13] or self.islands[14]:
                    if self.printingEnabled:
                        print(
                            f"Player {self.activePlayer} gains a free 1 cost feat due to the instant effect of The First Titan")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {self.activePlayer} gains a free 1 cost feat due to the instant effect of The First Titan\n")
                    if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1:
            ...
            case "COMPANION_INST_REINF":
                ...
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains a companion due to the instant effect of The Companion\n")
                self.players[self.activePlayer].companions.append(0)
                self.makeMove((Move.PASS, self.activePlayer, ()))
            case "SCEPTER_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains a scepter due to the instant effect of The Blacksmith's Scepter")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains a scepter due to the instant effect of The Blacksmith's Scepter\n")
                self.players[self.activePlayer].scepters.append(0)
            ...
            case "GREATGOLEM_INST":
                ...
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 2 maze moves due to the instant effect of The Great Golem\n")
                self.players[self.activePlayer].mazeMoves = 2
                self.blessingPlayer = self.activePlayer
                self.phase = Phase.RESOLVE_MAZE_MOVES
                if self.phase == Phase.ACTIVE_PLAYER_PERFORM_FEAT_1:
            ...
            case "TIMEGOLEM_INST":
                if self.printingEnabled:
                    print(
                        f"Player {self.activePlayer} gains 2 backwards maze moves due to the instant effect of The Great Golem")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 2 backwards maze moves due to the instant effect of The Great Golem\n")
                self.players[self.activePlayer].mazeMoves = -2
        ...
        if featPerformed:
            if not (feat in player.feats or (Data.isBoarFeat(feat) and Data.containsBoarFeat(player.feats)) or (
                ...
                if Data.getEffectLevel(player.allegiance) == -2:
                    ...
                    if self.printingEnabled:
                        print(
                            f"Player {player.playerID} gains 2 vp due to performing a heroic feat for the first time this game and their position on the allegiance track")
                    if self.loggingEnabled:
                        self.log.write(
                            f"Player {player.playerID} gains 2 vp due to performing a heroic feat for the first time this game and their position on the allegiance track\n")
                    player.gainVP(2)
            ...
            if Data.getEffectLevel(player.allegiance) == -2:
                if self.printingEnabled:
                    print(
                        f"Player {player.playerID} gains 5 vp due to being the first to perform a heroic feat this game and their position on the allegiance track")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {player.playerID} gains 5 vp due to being the first to perform a heroic feat this game and their position on the allegiance track\n")
                player.gainVP(5)
        ...
        while True:
            ...
            if found:
                ...
                if next[0]:
                    ...
                    if self.printingEnabled:
                        print(f"Player {playerID} gains 2 loyalty and 1 sun from a memory token")
                    if self.loggingEnabled:
                        self.log.write(f"Player {playerID} gains 2 loyalty and 1 sun from a memory token\n")
                    self.players[playerID].gainLoyalty(2)
                    self.players[playerID].gainSun(1, False)
            else:
                break
    ...
    def oust(self, player, actionNum):
        ...
        for feat in player.feats:
            if feat == Data.HeroicFeat.GREAT_BEAR:
                if self.printingEnabled:
                    print(f"Player {player.playerID} gains 3 vp due to the automatic effect of The Great Bear")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {player.playerID} gains 3 vp due to the automatic effect of The Great Bear\n")
        player.checkBears()
        for feat in self.players[self.activePlayer].feats:
            if feat == Data.HeroicFeat.GREAT_BEAR:
                if self.printingEnabled:
                    print(f"Player {self.activePlayer} gains 3 vp due to the automatic effect of The Great Bear")
                if self.loggingEnabled:
                    self.log.write(
                        f"Player {self.activePlayer} gains 3 vp due to the automatic effect of The Great Bear\n")
        self.players[self.activePlayer].checkBears()
    ...
    def logBoardState(self):
        self.log.write(f"Round: {self.round}\n")
        self.log.write(f"Player {self.activePlayer} is the active player.\n")
        self.log.write("Players:\n")
        for p in self.players:
            p.logPlayerInfo(self.log)
        self.log.write("Temple Pools:\n")
        i = 1
        for pool in self.temple:
            self.log.write(f"Pool {i}:\n")
            for face in pool:
                self.log.write(f"{face}\n")
            i += 1
        self.log.write("Islands:\n")
        i = 1
        for featPool in self.islands:
            self.log.write(f"Feat {i}:\n")
            if featPool:
                self.log.write(f"{featPool[0]} (x{len(featPool)})\n")
            else:
                self.log.write("Empty\n")
            i += 1
    ...
    def logPoints(self):
        self.log.write("Victory Points:\n")
        for player in self.players:
            self.log.write(f"Player {player.playerID}: {player.vp}\n")
...
class Player:
    ...
    def useHammerOrScepter(self, amount):  # spend amount on hammer or scepter, gain remainder as gold
    ...
    def gainDiceEffects(self, minotaur, sentinel, blessing, order):
        if order:  # default order
            self.gainDiceEffectsInternal(self.getDie1Result(), self.getDie2Result(), self.orChoice1, self.orChoice2,
                                         minotaur, sentinel, blessing)
        else:  # reverse order
            self.gainDiceEffectsInternal(self.getDie2Result(), self.getDie1Result(), self.orChoice1, self.orChoice2,
                                         minotaur, sentinel, blessing)

    def gainMazeDiceEffects(self, order):
        if order:  # default order
            self.gainDiceEffectsInternal(self.getMazeDie1Result(), self.getMazeDie2Result(), self.orMazeChoice1,
                                         self.orMazeChoice2, False, False, False)
        else:  # reverse order
            self.gainDiceEffectsInternal(self.getMazeDie2Result(), self.getMazeDie1Result(), self.orMazeChoice1,
                                         self.orMazeChoice2, False, False, False)
    ...
    def logPlayerInfo(self, log):
        log.write(f"Player {self.playerID}:\nGold: {self.gold}/{self.maxGold}\nSun: {self.sun}/{self.maxSun}\n")
        log.write(f"Moon: {self.moon}/{self.maxMoon}\nVictory Points: {self.vp}\n")
        log.write(f"Ancient Shards: {self.ancientShards}/6\n")
        log.write("Heroic Feats:\n")
        for feat in self.feats:
            log.write(f"{feat.name}\n")
        log.write("Dice:\n")
        log.write(f"{self.die1}\n")
        log.write(f"{self.die2}\n")
        if self.location == 0:
            log.write("Location: portal\n")
        else:
            log.write(f"Location: Island {self.location}\n")
        log.write(f"Maze location: {self.mazePosition}\n")
        log.write(f"Allegiance: {self.allegiance}\n")
        log.write(f"Hammer: {self.hammerTrack} / {self.getMaxHammer()}\n")
        for companion in self.companions:
            log.write(f"Companion: {companion}\n")
        for scepter in self.scepters:
            log.write(f"Scepter: {scepter}\n")
        log.write(f"Number of faces forged: {self.numForged}\n")
        log.write("Unforged Faces:\n")
        for face in self.unforgedFaces:
            log.write(f"{face}\n")
